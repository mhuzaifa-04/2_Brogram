import { workoutProgram as training_plan} from '../utilis/index.js';
...
export default function Grid() {
  ...
  function handleSave(index , data){
    const newObj ={ ...savedWorkouts, [index]:{ ...data  , isComplete:  !!data.isComplete || !!savedWorkouts?.[index]?.isComplete} }
  ...
  return (
    <div className="training-plan-grid">
      { Object.keys(training_plan).map((workout ,  workoutIndex) => {
        ...
        false : !completeWorkouts.includes(`${workoutIndex - 1}`)
        ...
        const icon = workoutIndex % 3 === 0 ? ( <i className="fa-solid fa-dumbbell" ></i>) : (
                  workoutIndex % 3 === 1 ? ( <i className="fa-solid fa-weight-hanging"></i> ) : ( <i className="fa-solid fa-bolt"></i> )
                );

        if (workoutIndex === selectedWorkout) {
          return (
            <WorkoutCard savedWeights={savedWorkouts?.[workoutIndex]?.weights} handleSave={handleSave} 
